...
describe("compress png images", () => {
  beforeAll(async () => {
    await execAsync("rm -rf tmp && mkdir tmp");
  });
...
describe("optimize png images that has been optimized", () => {
  ...
  test("skip optimized image", async () => {
    ...
    return execAsync(`bin/cli images/illust_optimized.png --out-dir tmp`).catch(
      e => expect(e.message).toMatch("There are no images to optimize")
    );
  });
});
...
describe("compress png and jpg images with regexp", () => {
  ...
  test("output some images to tmp directory", async () => {
    ...
    expect(rawJpgImage.byteLength).toBeGreaterThan(
      compressedJpgImage.byteLength
    );
    expect(rawPngImage.byteLength).toBeGreaterThan(
      compressedPngImage.byteLength
    );
  });
});
