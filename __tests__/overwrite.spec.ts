...
describe("overwrite images", () => {
  ...
  beforeAll(async () => {
    await execAsync("rm -rf tmp && mkdir tmp && cp images/* tmp");
  });
  ...
  test("optimized png image is given", async () => {
    ...
    return execAsync("bin/cli ./tmp/illust_optimized.png --overwrite").catch(
      e =>
        expect(e.message).toMatch(
          "./tmp/illust_optimized.png is already optimized"
        )
    );
  });
});
